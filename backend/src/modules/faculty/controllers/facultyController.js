<<<<<<< HEAD:backend/src/modules/faculty/controllers/facultyController.js
...
exports.updateFacultyProfile = async (req, res) => {
    try {
        ...
        const { school, department, designation, teachingAssignments } = req.body;
        ...
        // Update PG Profile as well
        if (pgUser.FacultyProfile) {
            if (school) pgUser.FacultyProfile.school = school;
            if (department) pgUser.FacultyProfile.department = department;
            if (designation) pgUser.FacultyProfile.designation = designation;
            await pgUser.FacultyProfile.save();
        }
...
exports.getTeachingMap = async (req, res) => {
    try {
        const { semester, QP_semester, section, branch } = req.query;

        // Fallback for "year" to support legacy frontend momentarily or if mixed
        const targetSemester = semester || QP_semester;

        if (!targetSemester || !section || !branch) {
            return res.status(400).json({ error: 'Missing parameters: semester, section, and branch are required' });
        }
        ...
        const mappedData = results.map(f => {
            const relevantAssignments = f.teachingAssignments.filter(a =>
                a.semester === targetSemester && a.section === section && a.branch === branch
            );
            return {
                facultyName: f.name,
...
>>>>>>> 54c65193bef0adb8c05bc7e519e5e1e947a58c34:backend/src/controllers/facultyController.js
