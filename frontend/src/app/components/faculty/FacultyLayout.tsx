...
export default function FacultyLayout({ children }: FacultyLayoutProps) {
    ...
    const hasPhoto = stella?.photo_url && stella.photo_url.length > 0;
                                ...
                                <div className="flex items-center space-x-4 cursor-pointer hover:opacity-80 transition-opacity">
                                    <div className="text-right hidden sm:block">
                                        ...
                                        <div className="text-[10px] text-gray-500 font-medium uppercase tracking-tighter">
                                            {stella?.department || user?.department || 'Department'}
                                        </div>
                                    </div>
                                    {hasPhoto ? (
                                        <img
                                            src={stella!.photo_url}
                                    ...
                                    <div className="flex items-center space-x-3 relative z-10">
                                        {hasPhoto ? (
                                            <img
                                                src={stella!.photo_url}
                                                alt={user?.name || 'Faculty'}
                                                className="w-12 h-12 rounded-full object-cover shadow-sm border-2 border-white/30"
                                            />
                                        ) : (
                                        ...
                                        <div className="overflow-hidden">
                                            ...
                                            {stella?.designation && (
                                                <p className="text-white/60 text-[9px] truncate mt-0.5">{stella.designation}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
